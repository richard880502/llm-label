...
class DatabaseConnection:
    ...
    def execute(self, sql: str, params: tuple | list | None = None) -> QueryResult:
        ...
        lastrowid = None
        needs_id = re.match(
            r"^\s*INSERT\s+INTO\s+(projects|tasks|api_tokens|mcp_oauth_clients|mcp_oauth_connections|mcp_oauth_tokens|assistant_conversations|assistant_messages)\b",
            translated,
            flags=re.IGNORECASE,
        )
...
SCHEMA_STATEMENTS = [
    ...
    """,
    "CREATE INDEX IF NOT EXISTS idx_task_result_snapshots_task ON task_result_snapshots(task_id)",
    """
